...
import { DialogModifNoteComponent } from '../dialog-modif-note/dialog-modif-note.component';
...
export class CellierComponent implements OnInit {
    ...
    displayedColumns: string[] = ["image","nom", "note", "quantite","pays_nom", "type", "millesime", "voir", "action" ];
    ...
    /** Bouton Modifier la bouteille */
    editNote(bouteille:IProduit): void {
        const dialogRef = this.dialog.open(DialogModifNoteComponent, {
            maxWidth: '370px',
            data:bouteille
        }).afterClosed().subscribe(res=>{
            this.getBouteillesDansCeCellier();
